...
export class AdminPanelComponent implements OnInit {
  ...
  enableCourt(id) {
    debugger;
    this.loading = true;
    this._httpService.masterPost("api/admin/enablecourt/", id).subscribe(result => {
      this.notifierService.notify('success', 'Court enabled successfully!');
      this.loading = false;
      for (const index in this.courtList) {
        if (this.courtList[index].Court_ID == id) {
          this.courtList[index].Status = true;
        }
      }
    }, error => {
      this.notifierService.notify('error', 'Whoops, something went wrong. Probably!');
  ...
  deleteCourt(content, id){
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
      if (result == "SaveClick") {
        this.loading = true;
        this._httpService.masterPost("api/admin/deletecourt/", id).subscribe(result => {
          this.notifierService.notify('success', 'Court deleted successfully!');
          this.loading = false;
          for (const index in this.courtList) {
            if (this.courtList[index].Court_ID == id) {
  ...
  disableCourt(id){
            ...
            this._httpService.masterPost("api/admin/disablecourt/", id).subscribe(result => {
              this.notifierService.notify('success', 'Court disabled successfully!');
              debugger;
              this.loading = false;
              for (const index in this.courtList) {
                if (this.courtList[index].Court_ID == id) {
                  this.courtList[index].Status = false;
                }
              }
            }, error => {
              this.notifierService.notify('error', 'Whoops, something went wrong. Probably!');
